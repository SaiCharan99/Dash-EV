...
def register(app):
    ...
    def cb_comparison(yr_range, regions, season):
        ...
        d = filter_demand(all_regions, yr_range, None)
        p = filter_prices(all_regions, yr_range)
        ...
        for i, reg in enumerate(all_regions):
            ...
            # normalise each axis to 0–100 for radar readability
            vals = [
                min(ren_pct, 100),
                min(avg_price / 2, 100),   # $/MWh → 0–100 (cap at $200)
                min(peak_gw * 10, 100),    # GW → 0–100 (cap at 10 GW)
