...
def _compute_kpis(yr_range, regions, season):
    from products.energy_dashboard.data import filter_demand, filter_generation, filter_prices

    yr0, yr1 = yr_range
    d  = filter_demand(list(regions), list(yr_range), season if season != 'All' else None)
    g  = filter_generation(list(regions), list(yr_range), season=season if season != 'All' else None)
    p  = filter_prices(list(regions), list(yr_range))
    dp = filter_demand(list(regions), [yr0, max(yr0, yr1 - 1)], None)
