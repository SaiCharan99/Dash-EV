...
def register(app):
    ...
    def cb_renewables(yr_range, regions, season):
        ...
        all_regions = regions or NEM_REGIONS
        g = filter_generation(all_regions, yr_range)
        ...
        annual_ren   = annual[annual.source_category == 'renewable'].groupby(
            ['region', 'year'])['generation_gwh'].sum().reset_index()
        ...
        # ── Heatmap: capacity factor by month × region (solar + wind) ─────────
        ren_sources = ['solar_utility', 'wind']
        g_cf = g[g.source.isin(ren_sources) & g.capacity_factor.notna()]
        if not g_cf.empty:
            ...
            fig2 = go.Figure(go.Heatmap(
                ...
                y=pivot.index,
                colorscale=[[0, '#EBF3FF'], [0.5, TEAL], [1, ENERGY_SOURCE_COLORS['solar_utility']]],
        ...
        opacities = [0.55 if v < 0 else 0.85 for v in values]
        bar_colors = [_rgba(c, op) for c, op in zip(colors, opacities)]
