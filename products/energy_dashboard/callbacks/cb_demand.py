...
def _compute_demand(yr_range, regions, season):
    ...
    d = filter_demand(regions_list, list(yr_range), None)
    if d is None or d.empty:
        return None, None, None

    # ── Stacked area: monthly GWh by region ──────────────────────────────────
    d_monthly = d.copy()
    d_monthly['period'] = (d_monthly['year'].astype(str) + '-'
                           + d_monthly['month'].astype(str).str.zfill(2))
    fig1 = go.Figure()
    ...
    d_heat['daily_gw'] = d_heat.apply(
        lambda r: r.demand_gwh / (calendar.monthrange(int(r.year), int(r.month))[1] * 24),
        axis=1,
    )
    ...
    fig3.update_layout(xaxis=dict(side='bottom'), margin=dict(l=48, r=80, t=16, b=40))
