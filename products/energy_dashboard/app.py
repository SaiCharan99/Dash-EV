        ...
        return html.Div([
            _row(
                _panel([
                    ...
                    _cap('Annual generation (GWh) for each fuel source as separate lines, aggregated across the selected regions.'),
                ], flex=2),
                _panel([
                    _ph('Current Year Mix', 'Share of total generation'),
                    dcc.Graph(id='gen-donut', config=cfg),
                    html.Div(id='gen-donut-legend'),
                    _cap('Generation share by fuel for the latest year in the selected range. Center value is the renewable percentage of total generation.', show_reset=False),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('Same annual data stacked: bar height equals total generation per year; segments are fuel shares.'),
                ], flex=2),
                _panel([
                    _ph('Monthly Generation', 'Last 3 years · stacked area'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='gen-monthly-area', config=cfg),
                    _cap('Monthly generation stacked by fuel for the last three years of the selected range.'),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('Annual emissions intensity per region (one line each) plus a dotted NEM-wide average. Computed from generation × standard fuel emission factors.'),
                ], flex=2),
                _panel([
                    _ph('Coal Retirement Runway', 'Major NEM coal units · closure schedule'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='gen-coal-runway', config=cfg),
                    _cap('Operating window (start year to scheduled closure) for ten major NEM coal units. Faded bars are already retired; the dashed line marks the current year. Not affected by filters.'),
                ]),
            ),
        ], style={'padding': P})
        ...
        return html.Div([
            _row(
                _panel([
                    ...
                    _cap('Horizontal bar from low to high LCOE estimate ($/MWh) for each technology; diamond marker is the mid value. Source: CSIRO GenCost 2024-25, not affected by filters.'),
                ]),
                _panel([
                    _ph('Cost Trajectories 2015–2030', '$/MWh · actuals + projections'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='econ-lcoe-trend', config=cfg),
                    _cap('Mid LCOE per year per technology; solid lines are historical, dotted lines are projections beyond 2024. Not affected by region or season filters.'),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('Annual average wholesale spot price across the selected regions, overlaid on shaded LCOE bands for solar and wind.'),
                ], flex=2),
                _panel([
                    _ph('Demand vs Average Cost', 'TWh (bars) · $/MWh (line)'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='econ-demand-cost', config=cfg),
                    _cap('Dual axis: bars are annual demand (TWh, left axis), line is annual average wholesale price ($/MWh, right axis).'),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('For each region, monthly average prices in the selected range are sorted from highest to lowest and plotted against percentile.'),
                ]),
                _panel([
                    _ph('Renewable Share vs Wholesale Price', 'One dot per state-year'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='econ-share-vs-price', config=cfg),
                    _cap('Scatter: x = renewable share (%), y = annual avg price ($/MWh). Each dot is one region in one year. Dashed line is the OLS fit with Pearson r in the legend.'),
                ]),
            ),
        ], style={'padding': P})
        ...
        return html.Div([
            _row(
                _panel([
                    ...
                    _cap('Grouped bars: renewable share (% of total generation) per region per year, within the selected year range.'),
                ], flex=2),
                _panel([
                    _ph('Capacity Factor Heatmap', 'Month × Region · solar & wind'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='ren-cf-heatmap', config=cfg),
                    _cap('Heatmap of average capacity factor (%) by region (rows) and calendar month (columns) for utility solar and wind combined.'),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('Difference in annual generation between the first and last year of the selected range, per fuel. Positive bars are growth, negative bars are decline.'),
                ], flex=2),
                _panel([
                    _ph('Solar vs Wind Complementarity', 'Monthly GWh · diversification proof'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='ren-solar-wind-corr', config=cfg),
                    _cap('Scatter: x = monthly solar GWh, y = monthly wind GWh. Each dot is one region in one month. Dashed line is the OLS fit with Pearson r in the legend.'),
                ]),
            ),
        ], style={'padding': P})
        ...
        return html.Div([
            _row(
                _panel([
                    ...
                    _cap('Grouped bars: generation (GWh) per fuel source for each selected region, using the last year of the selected range.'),
                ], flex=2),
                _panel([
                    _ph('State Radar', 'Renewable% · Price · Peak · Capacity Factor'),
                    html.Div(style={'height': '12px'}),
                    dcc.Graph(id='sc-radar', config=cfg),
                    _cap('Each region is one filled polygon across five normalised axes: renewable %, avg price, peak demand, solar CF, wind CF. Axes are scaled to 0–100 for comparability.'),
                ]),
            ),
            _row(
                _panel([
                    ...
                    _cap('One bubble per selected region: size scales with total demand (GWh), color encodes average wholesale price ($/MWh).'),
                ]),
            ),
        ], style={'padding': P})
